...
1. **Borrow.** From the repo: `treehouse get --lease --lease-holder claude`. Stdout carries only the slot path; banners go to stderr. Done when you hold the path.
...
- **Leases outlive the session.** A leased slot stays out of the pool until `treehouse return`. `treehouse status` lists leases with their holder.
