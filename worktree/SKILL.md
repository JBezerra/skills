...
3. **Start.** From the slot root: `NODE_CONFIG='{"port":<api>,"apiUrl":"http://localhost:<api>","apiUrlInternal":"http://localhost:<api>","appUrl":"http://localhost:<web>"}' yarn start`. `NODE_CONFIG` outranks every config file, `local.json` included. Web ignores it; vite takes the next free port after `3000` on its own and prints it (`Local: http://localhost:<web>/`). When that differs from `<web>`, restart with the corrected `appUrl`.
