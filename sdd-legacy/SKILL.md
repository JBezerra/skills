...
4. **On approval, commit.** Once the user is satisfied, commit exactly what's in the working tree at that point, scoped to this task. Use the `caveman-commit` skill to write the commit message, then apply whatever the repo's own commit conventions require on top (message style, sign-off lines, hook usage) — the repo's documented conventions win wherever the two disagree. `caveman-commit` applies to the commit message only, never to a PR title or description; those follow the `create-pr` skill and are not this loop's job. Then loop back to step 1 for the next task.
