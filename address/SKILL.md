...
2. **Check out.** By default, the PR's head branch in the current tree: a dirty tree stops here with a question; otherwise fetch, check out, and fast-forward to the remote. With `worktree`: when `git worktree list` shows a treehouse slot already on `<headRefName>`, `EnterWorktree` with its `path`; otherwise borrow a slot through the `worktree` skill and `git switch <headRefName>` there. Either way, fast-forward to the remote, then run the repo's setup (install, build) before any checks.
